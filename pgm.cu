...
void cuda_destroy_pgm_image(pgm_image *image)
{
  cudaFreeHost(image->matrix);
}
...
int32_t cuda_load_pgm_from_file(const char *filename, pgm_image *image)
{
  FILE *file = fopen(filename, "rb");
  ...
  size_t imageSize = image->height * image->width * sizeof(int32_t);

  cudaError_t cudaStatus = cudaHostAlloc((void **)&image->matrix, imageSize, cudaHostAllocDefault);
...
int32_t cuda_copy_pgm_image_size(const pgm_image *image, pgm_image *target)
{
  size_t size = image->width * image->height * sizeof(int32_t);

  cudaError_t cudaStatus = cudaHostAlloc((void **)&target->matrix, size, cudaHostAllocDefault);
  if (cudaStatus != cudaSuccess)
  {
    return ERR_MALLOC;
  }
