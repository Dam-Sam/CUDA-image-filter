                     ...
                     int32_t* output, int32_t width, int32_t height)
{
    ...
    timer.start_compute();
    kernel4<<<total_blocks, threads_per_block>>>(d_filter, dimension, d_input, d_output, d_min_array, d_max_array,
                                                 width, height);

    int32_t h_min, h_max;
    reduce_min_max(h_min_array, d_min_array, h_min, h_max_array, d_max_array, h_max, total_blocks);

    normalize4<<<total_blocks, threads_per_block>>>(d_output, width, height, h_min, h_max);
    timer.stop_compute();
